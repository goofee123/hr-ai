-- =============================================================================
-- COMPENSATION MODULE DATABASE MIGRATION
-- Sprint 13: Compensation Planning System
-- =============================================================================
...
CREATE INDEX IF NOT EXISTS idx_comp_rule_sets_tenant ON comp_rule_sets(tenant_id);
...
CREATE POLICY IF NOT EXISTS comp_cycles_tenant_isolation ON comp_cycles
    FOR ALL USING (tenant_id = current_setting('app.tenant_id', true)::uuid);

CREATE POLICY IF NOT EXISTS comp_dataset_versions_tenant_isolation ON comp_dataset_versions
    FOR ALL USING (tenant_id = current_setting('app.tenant_id', true)::uuid);

CREATE POLICY IF NOT EXISTS comp_employee_snapshots_tenant_isolation ON comp_employee_snapshots
    FOR ALL USING (tenant_id = current_setting('app.tenant_id', true)::uuid);

CREATE POLICY IF NOT EXISTS comp_rule_sets_tenant_isolation ON comp_rule_sets
    FOR ALL USING (tenant_id = current_setting('app.tenant_id', true)::uuid);

CREATE POLICY IF NOT EXISTS comp_rules_tenant_isolation ON comp_rules
    FOR ALL USING (tenant_id = current_setting('app.tenant_id', true)::uuid);

CREATE POLICY IF NOT EXISTS comp_scenarios_tenant_isolation ON comp_scenarios
    FOR ALL USING (tenant_id = current_setting('app.tenant_id', true)::uuid);

CREATE POLICY IF NOT EXISTS comp_scenario_results_tenant_isolation ON comp_scenario_results
    FOR ALL USING (tenant_id = current_setting('app.tenant_id', true)::uuid);

CREATE POLICY IF NOT EXISTS comp_worksheet_entries_tenant_isolation ON comp_worksheet_entries
    FOR ALL USING (tenant_id = current_setting('app.tenant_id', true)::uuid);

CREATE POLICY IF NOT EXISTS comp_comments_tenant_isolation ON comp_comments
    FOR ALL USING (tenant_id = current_setting('app.tenant_id', true)::uuid);

CREATE POLICY IF NOT EXISTS comp_approval_chains_tenant_isolation ON comp_approval_chains
    FOR ALL USING (tenant_id = current_setting('app.tenant_id', true)::uuid);

CREATE POLICY IF NOT EXISTS comp_exports_tenant_isolation ON comp_exports
    FOR ALL USING (tenant_id = current_setting('app.tenant_id', true)::uuid);

CREATE POLICY IF NOT EXISTS comp_audit_log_tenant_isolation ON comp_audit_log
    FOR ALL USING (tenant_id = current_setting('app.tenant_id', true)::uuid);
